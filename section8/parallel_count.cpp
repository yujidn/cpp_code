...
int main() {
  ...
  for (uint64_t size = 100; size <= vector_num; size *= 10) {
    ...
    t.restart();
    sum += std::count(vec.begin(), vec.end(), 1);

    t.print("count");
    t.restart();

    sum += std::count(std::execution::seq, vec_for_seq.begin(),
                      vec_for_seq.end(), 1);

    t.print("seq_count");
    t.restart();

    sum += std::count(std::execution::par, vec_for_par.begin(),
                      vec_for_par.end(), 1);

    t.print("par_count");
    t.restart();

    sum += std::count(std::execution::par_unseq, vec_for_par_unseq.begin(),
                      vec_for_par_unseq.end(), 1);

    t.print("par_unseq_count");
  }
